...
eventBus.on('connected', ws => {
	network.sendJustsaying(ws, 'custom', { service: 'udex' });
});

eventBus.on('custom_justsaying', async (ws, body) => {
	if (!body)
		return;
	if (body.service === 'udex') {
		assocWsPeers[ws.peer] = ws;
		console.log('added udex peer ' + ws.peer);
		ws.on('close', () => {
	...
	if (!assocWsPeers[ws.peer]) // ignore messages from non-udex peers
		return console.log("ignoring custom_justsaying from non-udex " + ws.peer);
...
eventBus.on('custom_request', async (ws, params, tag) => {
	...
	if (!assocWsPeers[ws.peer]) // ignore messages from non-udex peers
		return console.log("ignoring custom_request from non-udex " + ws.peer);
	switch (params.command) {
		...
		default:
			console.log('unknown command from udex peer: ' + params.command);
	}
});
