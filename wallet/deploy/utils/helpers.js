...
const getMongoURI = (user, password, environment) => {
	switch (environment) {
		case 'staging':
			return `mongodb+srv://${user}:${password}@ampstagingcluster0-qdjqg.mongodb.net/udex?retryWrites=true`
		case 'production':
			return `mongodb+srv://${user}:${password}@ampcluster0-xzynf.mongodb.net/udex?retryWrites=true`
		default: 
			return `mongodb+srv://${user}:${password}@ampcluster0-xzynf.mongodb.net/udex?retryWrites=true`
	}
}
